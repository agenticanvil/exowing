...
export class AsteroidBeltSystem implements WorldSystem {
    ...
    while (this.nextDistance <= context.railDistance + STREAM_AHEAD) {
      ...
      for (let index = 0; index < count; index++) {
        ...
        this.features.push({
          ...
          profile: (["monolith", "shard", "binary", "cratered"] as const)[
            Math.floor(random() * 4)
          ],
  ...
  render({ centerX, centerZ, time }: WorldRenderContext) {
    ...
    for (const feature of this.features) {
      ...
      view.rotation.y =
        -rail.heading + time * (0.025 + (feature.seed % 7) * 0.004);
...
export function createAsteroidGeometry(options: AsteroidGeometryOptions) {
  ...
  const craterCount =
    options.profile === "cratered" ? 5 : 2 + Math.floor(random() * 2);
  ...
  for (let index = 0; index < positions.count; index++) {
    ...
    const broadNoise =
      Math.sin(vertex.x * 3.1 + phase[0]) *
        Math.sin(vertex.y * 3.7 + phase[1]) +
    ...
    const light =
      ...
      Math.max(
        ...
        vertex
          .clone()
          .normalize()
          .dot(new THREE.Vector3(-0.5, 0.45, 0.72)),
      ) *
        0.26;
    const color = new THREE.Color(
      options.colors[(index + options.seed) % options.colors.length],
...
function createAsteroidFormation(
  ...
  for (let index = 0; index < fragmentCount; index++) {
    ...
    const fragment = new THREE.Mesh(
      createAsteroidGeometry({
        seed: hash(feature.seed + 41 + index),
    ...
    fragment.position.copy(
      direction.multiplyScalar(feature.radius * (1.2 + random() * 0.9)),
    );
