...
describe('WorldRuntime', () => {
  it('runs arbitrary composed systems without knowing their concrete types', () => {
    ...
    expect(firstStep).toHaveBeenCalledWith(expect.objectContaining({ railDistance: 125 }));
    expect(secondStep).toHaveBeenCalledWith(expect.objectContaining({ railDistance: 125 }));
  });
});
