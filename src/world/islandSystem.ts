...
export class IslandSystem implements WorldSystem {
  ...
  step(context: WorldStepContext) {
    ...
    removeWhere(this.islands, (island) => island.railDistance < context.railDistance - CLEANUP_MARGIN);
  }
...
export function islandField(options: IslandFieldOptions): WorldSystemDefinition {
...
function createIslandGeometry(seed: number, style: IslandStyle) {
  ...
  if (style === 'spires') {
    ...
    for (let index = 0; index < count; index++) {
      ...
      appendRock(positions, random, {
        centerX: Math.cos(angle) * distance, centerZ: Math.sin(angle) * distance,
        scaleX: width, scaleZ: depth, sides: 6 + Math.floor(random() * 3),
        rings: [[0.02, 0.82], [0.3, 0.72], [topY - 0.22, 0.61], [topY - 0.1, 0.34]],
        topY, jitter: 0.18, lean: 0.12,
      });
    }
  } else {
...
type RockSpec = {
  centerX: number; centerZ: number; scaleX: number; scaleZ: number; sides: number;
  rings: Array<[y: number, radius: number]>; topY: number; jitter: number; lean: number;
};
...
function appendRock(positions: number[], random: () => number, spec: RockSpec) {
  ...
  const triangle = (a: THREE.Vector3, b: THREE.Vector3, c: THREE.Vector3) => positions.push(a.x, a.y, a.z, b.x, b.y, b.z, c.x, c.y, c.z);
  for (let ring = 0; ring < vertices.length - 1; ring++) for (let index = 0; index < spec.sides; index++) {
    const next = (index + 1) % spec.sides;
    if ((index + ring) % 2 === 0) {
      triangle(vertices[ring][index], vertices[ring + 1][index], vertices[ring][next]);
      triangle(vertices[ring][next], vertices[ring + 1][index], vertices[ring + 1][next]);
    } else {
      triangle(vertices[ring][index], vertices[ring + 1][next], vertices[ring][next]);
      triangle(vertices[ring][index], vertices[ring + 1][index], vertices[ring + 1][next]);
    }
  }
  const top = new THREE.Vector3(spec.centerX + leanX, spec.topY, spec.centerZ + leanZ);
  for (let index = 0; index < spec.sides; index++) triangle(vertices.at(-1)![index], top, vertices.at(-1)![(index + 1) % spec.sides]);
}
