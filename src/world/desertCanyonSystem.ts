...
import { GROUND_SURFACE_Y, type WorldAttachContext, type WorldRenderContext, type WorldStepContext, type WorldSystem, type WorldSystemDefinition } from './worldSystem';
...
export class DesertCanyonSystem implements WorldSystem {
  ...
  private readonly rockMaterial = new THREE.MeshStandardMaterial({ vertexColors: true, roughness: 1, flatShading: true });
  ...
  constructor(private readonly options: DesertCanyonOptions) {
    this.rubbleMaterial = new THREE.MeshStandardMaterial({ color: options.rock[0], roughness: 1, flatShading: true });
  }
    ...
    while (this.nextDistance <= context.railDistance + STREAM_AHEAD) {
      ...
      for (const side of [-1, 1] as const) {
        ...
        this.features.push({
          ...
          arch: sideSeed % 9 === 0,
          profile: (['table', 'butte', 'weathered', 'shelf'] as const)[(sideSeed >>> 15) % 4],
        });
      }
  ...
  attach({ scene }: WorldAttachContext) {
    ...
    this.ground = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({ color: this.options.sand, roughness: 1 }));
  ...
  render({ centerX, centerZ }: WorldRenderContext) {
    ...
    for (const feature of this.features) {
      ...
      if (!view) {
        view = createFormation(feature, this.options.rock, this.rockMaterial, this.rubbleMaterial);
        this.views.set(feature.id, view);
        this.scene.add(view);
      }
      const position = railOffsetPosition(feature.railDistance, feature.side * feature.offset, GROUND_SURFACE_Y);
...
export function desertCanyon(options: DesertCanyonOptions): WorldSystemDefinition {
...
function createFormation(
  ...
  const main = new THREE.Mesh(createMesaGeometry({
    seed: feature.id,
    width: feature.width,
    depth: feature.depth,
  ...
  for (let index = 0; index < satelliteCount; index++) {
    ...
    const satellite = new THREE.Mesh(createMesaGeometry({
...
export function createMesaGeometry(options: MesaGeometryOptions) {
  ...
  const collapseStrength = options.profile === 'weathered' ? 0.26 + random() * 0.14 : 0.08 + random() * 0.1;
  ...
  for (let ring = 0; ring < heights.length; ring++) {
    ...
    for (let index = 0; index < segments; index++) {
      const angle = index / segments * Math.PI * 2;
      const angleDistance = Math.abs(Math.atan2(Math.sin(angle - collapseAngle), Math.cos(angle - collapseAngle)));
      const collapse = Math.exp(-Math.pow(angleDistance / collapseWidth, 2)) * collapseStrength * Math.sin(y * Math.PI);
      const verticalErosion = Math.sin(angle * 2 + phaseB + y * 5.2) * 0.035 * Math.sin(y * Math.PI);
      const radius = Math.max(0.34, radii[ring] * angularVariation[index] - collapse + verticalErosion);
      const ledgeSkew = (ring === 2 || ring === 5 || ring === 8) ? (random() - 0.5) * 0.035 : 0;
      positions.push(
  ...
  for (let index = 0; index < segments; index++) indices.push(topStart + index, topCenter, topStart + (index + 1) % segments);
...
function profileRadii(profile: MesaProfile) {
  if (profile === 'butte') return [1.02, 1, 0.91, 0.88, 0.84, 0.79, 0.74, 0.7, 0.66, 0.63];
  if (profile === 'weathered') return [1.08, 1.02, 0.93, 0.9, 0.82, 0.78, 0.69, 0.66, 0.57, 0.54];
  if (profile === 'shelf') return [1.08, 1.04, 0.97, 0.99, 0.86, 0.88, 0.73, 0.75, 0.61, 0.6];
...
function appendRockColor(
  ...
  const band = Math.min(palette.length - 1, Math.floor(((height * 5.4 + 0.15) % 1) * palette.length));
...
function createTalus(feature: CanyonFeature, random: () => number, material: THREE.MeshStandardMaterial) {
  ...
  for (let index = 0; index < count; index++) {
    ...
    scale.set(size * (0.8 + random()), size * (0.45 + random() * 0.45), size * (0.75 + random()));
