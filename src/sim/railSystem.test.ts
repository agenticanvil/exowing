...
import { railFrameAtDistance, SECTION_SPAN, TURN_ANGLE, TURN_LENGTH, TURN_START_DISTANCE } from './railSystem';
...
describe('railSystem', () => {
  ...
  it('continues smoothly across many procedural sections', () => {
    for (let section = 1; section < 12; section++) {
      ...
      const gap = Math.hypot(after.position.x - before.position.x, after.position.z - before.position.z);
