...
export class GameView {
  ...
  private readonly shotGeometry = new THREE.SphereGeometry(0.28, 8, 6);
  private readonly shotMaterial = new THREE.MeshBasicMaterial({ color: 0xfff37a });
  private readonly enemyShotMaterial = new THREE.MeshBasicMaterial({ color: 0xff4b9b });
  ...
  sync(sim: FlightSimulation) {
    ...
    syncProjectiles(this.scene, this.projectileViews, sim.projectiles, this.shotGeometry, this.shotMaterial, this.enemyShotMaterial);
...
function syncProjectiles(
  ...
  for (const state of states) {
    ...
    if (!mesh) {
      mesh = new THREE.Mesh(geometry, state.owner === 'player' ? playerMaterial : enemyMaterial);
