...
describe('playerPitch', () => {
  ...
  it('eases the rendered plane level near the edge it is approaching', () => {
    expect(Math.abs(playerPitch(FLIGHT_WINDOW.maxY - 0.5, 12)))
      .toBeLessThan(Math.abs(playerPitch(FLIGHT_WINDOW.maxY - 1.5, 12)));
    expect(playerPitch(FLIGHT_WINDOW.maxY, 12)).toBe(0);

    expect(Math.abs(playerPitch(FLIGHT_WINDOW.minY + 0.5, -12)))
      .toBeLessThan(Math.abs(playerPitch(FLIGHT_WINDOW.minY + 1.5, -12)));
